 ...
 *	\ingroup Utility
 *	This function converts the double-precision numbers internally to floating-point numbers. It is intended for use only where floating-point values are unavailable. For best performance, use the HIKSetEffectorPivotStateTQSfv() function instead wherever possible.
 *	\param		pRevert				Storage for a quaternion that needs to be preserved for subsequent HIKRevertEffectorPivotsTQSdv calls
 */
HIK_EXPORT void	 HIK_API HIKBlendEffectorPivotsTQSdv(HIKEffectorSetState *pEffectorSetState, int pEffectorId, double(*pT)[4], double(*pQ)[4], double(*pS)[4], double(*pIKPivotArray)[4], double *pReachTArray, double *pReachRArray, int pPivotCount, double pRevert[4]);

/**	\brief Blend the translation (T), quaternion rotation (Q), scaling (S) and IK constraints of Effector \p pEffectorId in HIKEffectorSetState \p pEffectorSetState between multiple values, each provided in a separate array.
*
*	\ingroup Utility1
*	Retrieve the original position and orientation of an effector from blended transforms (as produced by the HIKBlendEffectorPivotsTQSdv function for instance)
*	\param		pEffectorSetState	The HIKEffectorSetState containing \p pEffectorId. (blended transforms)
*	\param		pEffectorId			The unique ID of the Effector to be reverted.
*	\param		pT					An array that contains the different translation values that were blended for the Effector.
*	\param		pQ					An array of quaternions that contains the different rotation values that were blended for the Effector.
*	\param		pS					An array that contains the different scaling values that were blended for the Effector.
*	\param		pIKPivotArray		An array of IK pivots that were blended for the Effector. TODO: Not used yet
*	\param		pReachTArray		An array that controls the relative weight of the translation values provided for the Effector.
*	\param		pReachRArray		An array that controls the relative weight of the orientation values provided for the Effector.
*	\param		pPivotCount			The total number of sets of TQS and IK constraint values (i.e. the length of the \p pT, \p pQ, \p pT, \p pIKPivotArray, \p pReachTArray and \p pReachRArray parameters).
*	\param		pRevert				Revert quaternion data as returned by former HIKBlendEffectorPivotsTQSdv call for the Effector.
*/

HIK_EXPORT void	 HIK_API HIKRevertEffectorPivotsTQSdv(HIKEffectorSetState *pEffectorSetState, int pEffectorId, double(*pT)[4], double(*pQ)[4], double(*pS)[4], double(*pIKPivotArray)[4], double *pReachTArray, double *pReachRArray, int pPivotCount, double(*pRevert)[4]);
