...
#define HUMANIK_VERSION_MAJOR  2016
#define HUMANIK_VERSION_MINOR  5
#define HUMANIK_VERSION_THIRD  2


// Generate the version string based on HIK version.
#define HIK_EXPAND_TO_STRING_(x) #x
#define HIK_EXPAND_TO_STRING(x) HIK_EXPAND_TO_STRING_(x)

#define HUMANIK_VERSION_STRING                    \
	HIK_EXPAND_TO_STRING( HUMANIK_VERSION_MAJOR ) \
	"."                                           \
	HIK_EXPAND_TO_STRING( HUMANIK_VERSION_MINOR ) \
	"."                                           \
	HIK_EXPAND_TO_STRING( HUMANIK_VERSION_THIRD )
...
{ 
    ...
    HIKSolvingStepModifiers		    = 1 << 21,	//!< Activates Lock & Mirror modifiers.
	...
	HIKSolvingStepAll = HIKSolvingStepBodyPull | HIKSolvingStepContact | HIKSolvingStepContactApprox | HIKSolvingStepAllParts | HIKSolvingStepHipsTranslation | HIKSolvingStepRollExtraction | HIKSolvingStepModifiers
};
 ...
 *				You can determine which Nodes are missing by calling HIKValidateRequiredBone() on your HIKCharacterDefinition.
 */
   HIK_EXPORT HIKCharacter	    * HIK_API HIKCharacterCreateInPlace( const HIKCharacterDefinition *pCharacterDefinition, void* pBuffer, const char *pValidationString);
...
/**	\brief Set the translation limits for Node \p pNodeId.
 *	\ingroup CharacterLimits
 *	\param		pCharacter			The character you want to set the translation limits on.
 *	\param		pNodeId				The unique ID of the Node you want to set. See the ::HIKNodeId enumeration for possible values.
 *	\param		pMin				The minimum length that the joint can squash to.
 *	\param		pMax				The maximum length that the joint can stretch to.
 *	\param		pActive 			Indicates whether translation limits sjhould be activated or not for this joint. 
*/
HIK_EXPORT void	HIKSetTranslationLimitsf(HIKCharacter *pCharacter,int pNodeId, 
								float pMin, float pMax, bool pActive);

/**	@copydoc HIKSetTranslationLimitsf()
 *	\ingroup CharacterLimits
 */
HIK_EXPORT void	HIKSetTranslationLimitsd(HIKCharacter *pCharacter,int pNodeId, 
								double pMin, double pMax, bool pActive);
 ...
 *		- HIKSolveForEffectorSetHipsTranslation()
 *		- HIKSolveForEffectorSetRollExtraction()
 *		- HIKSolveForEffectorSetModifiers()
 *
...
/**	\ingroup IK
 *	\brief Call the Mirror and Lock modifiers for character \p pCharacter.
 *
 *	\param			pCharacter			Character to solve.
 *	\param			pEffectorSetState	HIKEffectorSetState containing the IK goal points and IK constraint values.
 *	\param[in,out]	pCharacterState		HIKCharacterState containing the initial FK pose for your character, which may influence the IK solution.
 *	\param			pPropertyState		HIKPropertySetState containing all character properties for the character.
 */
HIK_EXPORT void	 HIK_API HIKSolveForEffectorSetModifiers(const HIKCharacter *pCharacter, HIKCharacterState *pCharacterState, HIKEffectorSetState *pEffectorSetState, const HIKPropertySetState *pPropertyState);
...
/**	\brief Set the translation (T), quaternion rotation (Q) and scaling (S) in global or local space of Node \p pNodeId in HIKCharacterState \p pCharacterState. Values are expressed using separate arrays and update dependent nodes.
 *
 *	\ingroup CharacterStateSetupWrite
 *	\param		pCharacter			HIKCharacter for which you want to set the data values.
 *	\param		pCharacterState		HIKCharacterState in which you want to set the data values.
 *	\param		pNodeId				Unique ID of the Node whose data you want to set. See the ::HIKNodeId enumeration for possible values.
 *	\param		pT					Translation value to set, in global or local space.
 *	\param		pQ					Rotation value to set, in global or local space.
 *	\param		pS					Scaling value to set, in global or local space.
 *  \param      pLocal              Specify whether pT,pQ and pS are local or not (if not, they are global)
 */
HIK_EXPORT void HIKSetNodeStateTQSfvComputeDependent(const HIKCharacter* pCharacter, HIKCharacterState *pCharacterState, int pNodeId, const float * pT, const float * pQ, const float * pS, const bool pLocal);
...
/**	\brief Compute a local quaternion from an euler angle, using the DOFs structures (pre-rotation, rotation order, ...)
 *	\ingroup Utility
 */

HIK_EXPORT void	HIK_API HIKGetLocalQFromDOFfv(const HIKCharacter *pCharacter, int pNodeId, const float pEuler[4], float pQuat[4]);

/**	\brief Compute the euler angle equivalent of a local quaternion, using the DOFs structures (pre-rotation, rotation order, ...)
 *	\ingroup Utility
 */

HIK_EXPORT void	HIK_API HIKGetDOFFromLocalQfv(const HIKCharacter *pCharacter, int pNodeId, const float pQuat[4], float pEuler[4]);

/**	\brief Compute the result of a point tranformed by a quaternion.
 *	\ingroup Utility
 */

HIK_EXPORT void	HIK_API HIKQTransform(const float quat[4], const float p[4], float q[4]);

/**	\brief Compute euler angles from a quaternion (default XYZ order)
 *	\ingroup Utility
 */

HIK_EXPORT void	HIK_API HIKEulerFromQuat(const float quat[4], float euler[4]);

/**	\brief Allow setting original length for joints from CharacterState instead of characterization pose (default)
 *	\ingroup Utility
 */

HIK_EXPORT void HIK_API HIKSetOriginalSnSLength(HIKCharacter *pCharacter, bool pFromCharacterizationPose, HIKCharacterState *pCharacterState);
...
#endif
#define HIK_INTERNAL_PLACEHOLDER
#endif
